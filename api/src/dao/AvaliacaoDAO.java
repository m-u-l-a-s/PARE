...
//    }
        ...
        String sql = "SELECT avaliacao_nome FROM api.avaliacao where DATE(avaliacao_data_final) = curdate()";
        // select antigo: SELECT avaliacao_nome FROM api.avaliacao
        // select novo: SELECT avaliacao_nome FROM api.avaliacao where DATE(avaliacao_data_final) = curdate()        
        
        try (PreparedStatement stmt = connection.prepareStatement(sql)){
            try (ResultSet rs = stmt.executeQuery()){
                while(rs.next()){
                    String nomeTrab = rs.getString("avaliacao_nome");
                    listNomeTrab.add(nomeTrab);
                }
            }
        } catch (SQLException e){throw new RuntimeException(e);
